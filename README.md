...
**Revisão burst a burst.** Zoom e arrasto não bastam para conferir dezenas de onsets; a barra abaixo do traçado percorre os bursts um a um. **Revisar automaticamente** verifica cada marcador no envelope do canal âncora e sinaliza em âmbar os suspeitos, com o motivo: burst fraco (pico < 3× a linha de base), sem silêncio antes, atividade imediatamente antes do marcador (o início real seria anterior), envelope ainda silencioso logo depois (marcador antes do burst), marcador muito distante da subida do envelope (> 60 ms depois ou > 35 ms antes) ou intervalo < 250 ms ao vizinho (marcação dupla). A revisão automática não é um terceiro estimador de início — critérios legítimos discordam em dezenas de ms em bursts com precursor lento — e por isso só aponta situações objetivamente problemáticas. Em seguida, **▶ / ◀** (teclas **N**/**P**) mostram cada burst numa janela fixa (±100 ms a ±2 s; ±250 ms por padrão) com o marcador já selecionado; **suspeito ▶** (**S**) salta ao próximo sinalizado; em cada burst, arraste a linha ou use ←/→ para corrigir e então **Enter** aceita, **R** refina pelo critério de início configurado e **Delete** remove. Marcadores aceitos ou editados ficam verdes. O **mapa do registro** (faixa abaixo do traçado, com o envelope comprimido, os marcadores coloridos pelo estado e a janela atual) permite saltar a qualquer trecho com um clique ou arrastar a janela; **PgUp**/**PgDn** percorrem o traçado por páginas e **Home**/**End** vão aos extremos. O relatório registra quantos marcadores foram aceitos, corrigidos e removidos.
